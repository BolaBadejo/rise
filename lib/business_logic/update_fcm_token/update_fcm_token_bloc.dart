...
import '../../data/repositories/update_fcm_token_repo.dart';

part 'update_fcm_token_event.dart';
part 'update_fcm_token_state.dart';

class UpdateFcmTokenBloc extends Bloc<UpdateFcmTokenEvent, UpdateFcmTokenState> {
  final UpdateFcmTokenRepository updateFcmTokenRepository;
  UpdateFcmTokenBloc(this.updateFcmTokenRepository) : super(UpdateFcmTokenInitialState()) {
    on<UpdateFcmTokenEvent>((event, emit) async {
      if (event is LoadUpdateFcmTokenEvent) {
        emit(UpdateFcmTokenLoadingState());
        try {
          final updateFcmTokenResponse = await updateFcmTokenRepository.fetchUpdateFcmTokenData();
